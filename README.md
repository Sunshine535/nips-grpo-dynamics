...
This repository unifies two lines of work on **GRPO (Group Relative Policy Optimization)** stability after merging the **RLBalance** and **HalluZero** research tracks:

- **RLBalance** contributes a **theoretical and empirical map** of training dynamics in the **(α, β) plane**: α controls the relative weight of **positive** (correct) signals and β scales **negative** (incorrect) signals. We estimate **stable**, **unstable**, and **collapsed** regions and visualize them as a **phase diagram** over this plane.
- **HalluZero** contributes **practical gradient-level fixes** for **zero-score collapse**—the failure mode where a large fraction of samples receive **zero reward**, so their policy-gradient contribution vanishes and optimization stalls or becomes noisy. We study **four reshaping strategies** (`clip`, `temperature`, `curriculum`, `relabel`) that reweight or reschedule those contributions.

Together, the story is: **characterize where GRPO breaks** (phase diagram and collapse zones), then **target that regime** with **zero-score-aware gradient reshaping** and **curriculum schedules** in α and β.
...
```bash
cd /path/to/nips-grpo-dynamics

# One-shot environment (PyTorch + CUDA wheels + Python deps); see setup.sh
bash setup.sh
...
| Path | Content |
|------|---------|
| `checkpoints/` | Phase-diagram and baseline GRPO checkpoints |
| `results/phase_diagram/` | Per-point `eval_*.json` from `eval_phase_point.py` |
| `results/zero_score_sweep/sweep/...` | HalluZero runs (layout matches `run_diagnostic_analysis.py`) |

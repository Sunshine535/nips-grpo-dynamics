...
inputs during log-prob computation, the model carries stale rope_deltas from the
generation step, causing a RuntimeError shape mismatch.
...
Three-level defense:
  ...
  3. TrainerCallback: reset model state between generation and training steps.
...
Usage:
    ...
    trainer = GRPOTrainer(..., callbacks=[ClearRopeDeltasCallback()])
"""
...
def _find_model_class():
    """Locate the Qwen3.5 model class that owns compute_3d_position_ids."""
    module_names = [
...
def apply_qwen35_text_only_patch() -> bool:
    """
    Monkey-patch Qwen3.5's compute_3d_position_ids for text-only safety.

    Key fix: unconditionally clear self.rope_deltas BEFORE calling the original
    function, then catch any RuntimeError and fall back to 1D positions.
    """
...
def patch_model_instance(model) -> bool:
    """
    Register a forward pre-hook on the inner Qwen3.5 model to clear
    rope_deltas before every forward call.  Works even if the class-level
...
class ClearRopeDeltasCallback(TrainerCallback):
    """
    Defense-in-depth: clear rope_deltas on every module that has it,
    both at training start and before each step.
    """

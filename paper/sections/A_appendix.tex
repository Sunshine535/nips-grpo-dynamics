...
The maximum positive advantage occurs when $r_i = 1$ and all other responses have $r_j = 0$: $\Zp = 0.5$, so $A_i = 0.5/0.5 = 1$.
The minimum negative advantage occurs symmetrically when $r_i = 0$ and all others have $r_j = 1$: $\Zm = 0.5$, so $A_i = -0.5/0.5 = -1$.
Thus $A_i \in [-1, 1]$.
In all-correct groups, $A_i = 1/G$; in all-incorrect groups, $A_i = -1/G$.
...
Greedy decoding with max 256 new tokens. Answer extracted via ``\#\#\#\#'' delimiter regex with a last-number fallback. Accuracy computed as exact string match of the extracted answer against the ground truth on the full GSM8K test set ($n = 1{,}319$).
